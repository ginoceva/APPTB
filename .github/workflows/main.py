...
# Detectamos si estamos en Android para ajustar rutas (preparación para APK)
try:
...
DB_PATH = os.path.join(BASE_DIR, 'datos_logistica.db')
USUARIOS_PATH = os.path.join(BASE_DIR, 'Usuarios.xlsx')

# En Android guardamos en una carpeta pública si es posible, o local
if IS_ANDROID:
    REPORTE_PATH = os.path.join(BASE_DIR, 'Reporte_Escaneos.xlsx') 
else:
    REPORTE_PATH = os.path.join(BASE_DIR, 'Reporte_Escaneos.xlsx')
...
def main(page: ft.Page):
    ...
    # ==========================================
    # HERRAMIENTAS GLOBALES (SE CARGAN 1 VEZ)
    # ==========================================
    
    # 1. Función de resultado del selector
    def on_dialog_result(e):
        if e.files:
            ...
            # Si estamos en la pantalla listado, refrescamos para ver el botón verde
            # Usamos un truco seguro: si existe la función de actualizar, la llamamos
            if page.route == "/listado":
                mostrar_listado()
            else:
                page.update()

    # 2. Crear el FilePicker Globalmente
    file_picker = ft.FilePicker(on_result=on_dialog_result)
    
    # 3. Agregarlo al overlay UNA SOLA VEZ AL INICIO
    ...
    # --- ENVÍO DE CORREO (MÉTODO MANUAL/WEB) ---
...
(Por favor adjuntar Excel y Fotos manualmente desde la carpeta abierta)
    ...
    def limpiar_pantalla():
        # IMPORTANTE: Solo limpiamos los controles de la página, NO el overlay
        page.clean()
    ...
    def mostrar_listado():
        ...
        # No agregamos FilePicker aquí, ya está en main()
        ...
        # Botones llaman al FilePicker global usando page.data
